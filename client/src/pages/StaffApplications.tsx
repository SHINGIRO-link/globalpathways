...
import { Download, FileDown, FileText, Loader2, RefreshCw, Search, ShieldCheck, X } from "lucide-react";
...
export default function StaffApplications() {
  ...
  return <main className="staff-applications-page"><div className="container"><AccountControls />
    ...
    <div className="staff-toolbar"><div className="staff-search"><Search size={16} /><input type="search" aria-label="Search applicant submissions" value={q} onChange={(event) => setQ(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void load(); }} placeholder="Search applicant name or email" />{q && <button type="button" className="staff-search-clear" aria-label="Clear applicant search" onClick={() => { setQ(""); void load(); }}><X size={15} /></button>}</div><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All application statuses</option>{data?.statuses.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><select value={paymentStatus} onChange={(event) => setPaymentStatus(event.target.value)}><option value="">All payment statuses</option>{data?.payment_statuses.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><button className="button button-outline compact" onClick={() => void load()}><RefreshCw size={15} /> Refresh</button><a className="button button-primary compact" href={getStaffApplicationsExportUrl()} download><Download size={15} /> Download CSV</a><label className="staff-date-filter"><span>From</span><input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)} /></label><label className="staff-date-filter"><span>To</span><input type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)} /></label><a className="button button-outline compact" href={getStaffDocumentsExportUrl({ status, date_from: dateFrom, date_to: dateTo })} download><FileDown size={15} /> Download all documents</a></div>
